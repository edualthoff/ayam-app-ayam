...
import { auditTime, debounce, debounceTime, distinct, distinctUntilKeyChanged, map, tap } from 'rxjs/operators';
...
export class ListInfoPage implements OnInit {
  ...
  listaInformativo(tipo: string = '') {
    ...
    this.httpInfo.findParameterNameOrAll('', tipo, 'asc', this.page, this.size).pipe(auditTime(20000),tap(pp => {
