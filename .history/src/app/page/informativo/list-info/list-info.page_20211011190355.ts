...
export class ListInfoPage implements OnInit {
  ...
  listaInformativo(tipo: string = '') {
    ...
    this.httpInfo.findParameterNameOrAll('', tipo, 'asc', this.page, this.size).pipe(debounceTime(100000),tap(pp => {
      this.infoAll.push(...pp.content);

    })
    ).subscribe(() =>       console.log("vv res map - "+this.spinnerLoadService.loading$)
    );
  }
