...
export class ListInfoPage implements OnInit {
  ...
  listaInformativo(tipo: string = '') {
    ...
    this.httpInfo.findParameterNameOrAll('', tipo, 'asc', this.page, this.size).pipe(tap(pp => {
      ...
      console.log("ee c "+ pp.content.length)
    })
    ).subscribe(x => {
      console.log("ee subscribe "+ x.content.length)
      this.infoAll = [];
      from(x.content).pipe(distinctUntilKeyChanged('id')).subscribe(x => { console.log("ee cvvv "+ x.id); this.infoAll.push(x)})
    });
  }
