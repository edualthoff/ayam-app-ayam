...
export class AuthService {
  ...
  refresh() {
    return this.loginService.refresh(this.token.headerValue().replace('(bearer|bBearer)', '').trim()).pipe(
  ...
  logout() {
    ...
    this.token.clear();
    return this.loginService.logout(this.token.headerValue().replace('(bearer|bBearer)', '').trim()).pipe(
      map(() => !this.check())
    );
  }
