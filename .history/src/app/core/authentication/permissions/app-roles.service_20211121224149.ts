...
import { notEmptyValue, notUndefinedOrNull, transformStringToArray } from '../../utils/helpers';
...
export class AppRolesService implements OnDestroy {
  ...
  private rolesToken() {
    return this.tokenService.changed().pipe(tap(() => this.flush()),
    mergeMap(x => iif(() => !!x, of(x) )),
    map(x => {this.addRoles(x.roles())})
    ).subscribe();
  }
  ...
  private hasRoleKey(roleName: string[]): Promise<boolean> {
    const promises: Observable<boolean>[] = roleName.map((key) => {
      console.log(" a "+notUndefinedOrNull(this.rolesSource.value)+" "+JSON.stringify(this.rolesSource.value))
      const hasValidationFunction = notUndefinedOrNull(this.rolesSource.value) && !!this.rolesSource.value[key];
