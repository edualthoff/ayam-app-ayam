...
export class AppPermissionsDirective implements OnInit, OnChanges {
    ...
    if (onlyChanges || exceptChanges) {
      ...
      merge(this.rolesService.roles$)
        ...
        .subscribe(() => {
          ...
          if(notEmptyValue(this.appPermissions) || !notEmptyValue(this.appPermissions)) {
            console.log("notEmptyValue" )
            this.validateAllPermissionsNotVerifyRole()
            return;
          }
          console.log("handleAuthorisedPermission merge" )
  ...
  private validateAllPermissionsNotVerifyRole(){

    if (!!this.rolesService.getRoles()) {
      console.log("notEmptyValue 2" )
      this.handleAuthorisedPermission(this.getAuthorisedTemplates());
    } else {
      console.log("notEmptyValue 3" )
