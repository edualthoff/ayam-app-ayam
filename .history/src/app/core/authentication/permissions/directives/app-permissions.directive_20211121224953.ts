...
export class AppPermissionsDirective implements OnInit, OnChanges {
  ...
  @Input() appPermissions: TemplateRef<any>;
  ...
  constructor(private viewContainer: ViewContainerRef, private changeDetector: ChangeDetectorRef,
   @Inject(TemplateRef) private templateRef: TemplateRef<any>, private rolesService: AppRolesService) { }

  ngOnChanges(changes: SimpleChanges): void {
    const onlyChanges = changes.appPermissionsOnly;
    const exceptChanges = changes.appPermissionsExcept;
    const permissionsAll = changes.appPermissions;
    console.log("ngOnChanges permissionsAll "+ this.appPermissions+"' "+permissionsAll)
    if(permissionsAll == undefined) {
      console.log("ngOnChanges "+permissionsAll)
      if (permissionsAll && permissionsAll.firstChange) {
        return;
      }
      if (!!this.rolesService.roles$) {
        this.handleAuthorisedPermission(this.appPermissions || this.templateRef);
      } else {
        this.handleUnauthorisedPermission(this.appPermissions);
      }
    }
