...
export class AuthGuard implements CanActivate, CanActivateChild {
  ...
  private transformPermission(permissions: PermissionsRouterData, route: ActivatedRouteSnapshot | Route,
    ...
    const only = noUndefined(permissions.only) ? transformStringToArray(permissions.only) : '' ;
    const except = noUndefined(permissions.except) ? transformStringToArray(permissions.except) : '';
