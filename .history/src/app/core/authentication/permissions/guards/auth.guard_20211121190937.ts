...
export class AuthGuard implements CanActivate, CanActivateChild {
  ...
  private hasPermissions(route: ActivatedRouteSnapshot | Route, state?: RouterStateSnapshot) {
    ...
    if (this.isParameterAvailable(permission.only)) {
      console.log("aq token only " + JSON.stringify(!this.passingOnlyPermissionsValidation(permission.only)))
  ...
  private passingOnlyPermissionsValidation(roles: string | string[]): Promise<boolean> {
    return Promise.resolve(this.roleValidation(roles).then(x => {
      if (!x) {
  ...
  private permissionsValidation(): Promise<boolean> {
    console.log("perr 1 "+!!this.roles.roles$)
    return Promise.resolve(this.roles.roles$.toPromise().then(x => {
      console.log("perr "+!!x)
      if(!x) {
        this.redirectToAnotherRoute();
        return false;
